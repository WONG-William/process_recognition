    ...
    def ransac_fit(self, pts_s, pts_t):
        ...
        for i in range(ITER_NUM):
            ...
            if not(residual is None):
                ...
                inliers_tmp = np.where(residual < 2)
                #inliers_tmp = np.where(residual < 2)
                ...
                #if inliers_num_tmp > self.K:
                if inliers_num_tmp > 6:
                    # Update the number of inliers
                    inliers_num = inliers_num_tmp
                    bound_list.append(inliers_tmp)
                    # Set returned value
                    #inliers = inliers_tmp
                    inliers = np.hstack((inliers, inliers_tmp[0]))
